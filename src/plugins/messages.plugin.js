export default {
  install: (app, options) => {
    app.config.globalProperties.$showMessage = (message) => {
      ...
      div2.className = "w-64 px-4 py-3 text-sm text-white bg-gray-600 rounded-md shadow-md animate-flyin";
    ...
    app.config.globalProperties.$showError = (error) => {
      function close() {
        div2.classList.remove("animate-flyin");
        div2.classList.add("animate-flyout");
        setTimeout(() => { div3.removeEventListener('click', close); div1.remove(); }, 500);
      };
      ...
      div2.className = "w-64 relative px-4 py-3 text-sm text-white bg-red-600 rounded-md shadow-md animate-flyin";
      div3.className = "absolute top-1 right-1";
      div2.innerHTML = `<p>[ОШИБКА]: ${(options.errors[error] || 'Неизвестная ошибка')}</p>`;
